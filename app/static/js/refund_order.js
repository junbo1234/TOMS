...
class RefundOrderManager {
    ...
    createDetailCard(index, fields) {
        ...
        cardHeader.innerHTML = `
            <h6 class="mb-0 text-primary">
                <i class="fas fa-box me-2"></i>退款明细 ${index + 1}
            </h6>
            <div class="btn-group btn-group-sm">
                <button type="button" class="btn btn-outline-secondary btn-sm" onclick="refundManager.copyDetail(${index})">
                    <i class="fas fa-copy"></i>
                </button>
                <button type="button" class="btn btn-outline-danger btn-sm" onclick="refundManager.removeDetail(${index})">
    ...
    async handleSubmit(e) {
        ...
        try {
            ...
            for (let i = 0; i < detailCount; i++) {
                ...
                const applyNumInput = document.getElementById(`applyNum${i}`);
                
                if (platformNoInput) formData.set(`platformNo${i}`, platformNoInput.value);
                if (applyNumInput) formData.set(`applyNum${i}`, applyNumInput.value);
            }

            const response = await fetch('/refund_order/submit', {
                method: 'POST',
                headers: {
    ...
    updateJsonPreview() {
        try {
            ...
            const refundData = {
                ...
                platformRefundNo: data.platformRefundNo,
                ...
                platformStatus: data.platformStatus || '10',
                omsStatus: data.omsStatus || '1',
            ...
            for (let i = 0; i < detailCount; i++) {
                ...
                const applyNum = data[`applyNum${i}`] || '';
                
                console.log(`明细 ${i+1}:`, { platformNo, applyNum }); // 调试信息
                
                const detail = {
                    ...(this.preset.salesOrderRefundApplyDetailList && this.preset.salesOrderRefundApplyDetailList[0] || {}),
                    platformNo: platformNo,
                    applyNum: applyNum
                };
                refundData.salesOrderRefundApplyDetailList.push(detail);
            }

            console.log('最终JSON明细数量:', refundData.salesOrderRefundApplyDetailList.length); // 调试信息

            this.jsonPreview.textContent = JSON.stringify(refundData, null, 2);
    ...
    setupAutoSave() {
        ...
        this.form.addEventListener('input', () => {
            ...
            const data = Object.fromEntries(formData.entries());
            localStorage.setItem('refundOrderForm', JSON.stringify(data));
        });

        // 恢复保存的数据
        const savedData = localStorage.getItem('refundOrderForm');
        if (savedData) {
            try {
    ...
    saveToHistory(data) {
        const history = JSON.parse(localStorage.getItem('refundOrderHistory') || '[]');
        history.unshift({
            ...data,
        ...
        localStorage.setItem('refundOrderHistory', JSON.stringify(history));
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('DOM加载完成，开始初始化 RefundOrderManager'); // 调试信息
    refundManager = new RefundOrderManager();
    console.log('RefundOrderManager 初始化完成:', refundManager); // 调试信息
});
