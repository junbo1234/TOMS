...
class AllocationOutManager {
    ...
    createDetailCard(index, fields) {
        ...
        cardHeader.innerHTML = `
            ...
            <div class="btn-group btn-group-sm">
                <button type="button" class="btn btn-outline-secondary btn-sm" onclick="allocationManager.copyDetail(${index})">
                    <i class="fas fa-copy"></i>
                </button>
                <button type="button" class="btn btn-outline-danger btn-sm" onclick="allocationManager.removeDetail(${index})">
    ...
    async handleSubmit(e) {
        ...
        try {
            ...
            const response = await fetch('/allocation_out/submit', {
                method: 'POST',
                headers: {
    ...
    updateJsonPreview() {
        try {
            ...
            for (let i = 0; i < detailCount; i++) {
                ...
                allocationData.callbackResponse.orderLines.push(detail);
            }

            console.log('最终JSON明细数量:', allocationData.callbackResponse.orderLines.length); // 调试信息

            this.jsonPreview.textContent = JSON.stringify(allocationData, null, 2);
    ...
    setupAutoSave() {
        ...
        this.form.addEventListener('input', () => {
            ...
            const data = Object.fromEntries(formData.entries());
            localStorage.setItem('allocationOutForm', JSON.stringify(data));
        });

        // 恢复保存的数据
        const savedData = localStorage.getItem('allocationOutForm');
        if (savedData) {
            try {
    ...
    saveToHistory(data) {
        const history = JSON.parse(localStorage.getItem('allocationOutHistory') || '[]');
        history.unshift({
            ...data,
        ...
        localStorage.setItem('allocationOutHistory', JSON.stringify(history));
    }
}

// 初始化
let allocationManager;
document.addEventListener('DOMContentLoaded', () => {
    console.log('DOM加载完成，开始初始化 AllocationOutManager'); // 调试信息
    allocationManager = new AllocationOutManager();
    console.log('AllocationOutManager 初始化完成:', allocationManager); // 调试信息
});

// 全局函数（用于HTML中的onclick）
window.allocationManager = allocationManager;